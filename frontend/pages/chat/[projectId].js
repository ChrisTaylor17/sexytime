...
export default function ChatRoom() {
  ...
  const fetchProject = async (projectId) => {
    try {
      const response = await axios.get(`${process.env.BACKEND_URL}/api/projects/${projectId}`)
  ...
  const submitTaskProof = async () => {
    ...
    const alias = localStorage.getItem('userAlias')
    try {
      const response = await axios.post(`${process.env.BACKEND_URL}/api/verify-task`, {
        alias,
        projectId,
        proof: taskProof
      })
  ...
  const simulateQRScan = async () => {
    const alias = localStorage.getItem('userAlias')
    try {
      await axios.post(`${process.env.BACKEND_URL}/api/checkin`, {
        alias,
        projectId
      })
